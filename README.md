...
* Para instalar yarn, es se hace sobre cada version de Node, es decir nos movemos a la version de node eh hacemos la instalacion de Yarn
    * ``npm install -g yarn``
* Instalacion de Angular CLI se hace sobre cada version de Node instalado en nvm `Ojo, si tecleas **npm install -g @angular/cli** se intentara instalar la ultima version por lo tanto puede que mande error, entonces se tiene que instalar en base a la version de Angular con la que vas a trabajar` [Compatibilidad Angular-Node](https://angular.dev/reference/versions)
...
# Nueva combinacion de teclas con AutoHotkey
